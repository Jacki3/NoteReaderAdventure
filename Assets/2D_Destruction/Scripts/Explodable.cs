...
public class Explodable : MonoBehaviour
{
    ...
    /// </summary>
    private void generateFragments(SpriteRenderer _spriteRenderer)
    {
        ...
        switch (shatterType)
        {
            case ShatterType.Triangle:
                fragments =
                    SpriteExploder
                        .GenerateTriangularPieces(gameObject,
                        extraPoints,
                        subshatterSteps);
                break;
            case ShatterType.Voronoi:
                fragments =
                    SpriteExploder
                        .GenerateVoronoiPieces(gameObject,
                        extraPoints,
                        subshatterSteps);
                break;
            default:
        ...
        foreach (GameObject p in fragments)
        {
            if (p != null)
            {
                ...
                p.GetComponent<Renderer>().sortingLayerName =
                    _spriteRenderer.sortingLayerName;
                p.GetComponent<Renderer>().sortingOrder =
                    _spriteRenderer.sortingOrder;
                var destroyOverTime = p.AddComponent<DestroyOverTime>();
            }
        }
    ...
    private void setPolygonsForDrawing()
    {
        ...
        foreach (GameObject frag in fragments)
        {
            ...
            foreach (Vector2
                point
                in
                frag.GetComponent<PolygonCollider2D>().points
            )
            {
                Vector2 offset =
                    rotateAroundPivot((Vector2) frag.transform.position,
    ...
    private Vector2
    rotateAroundPivot(Vector2 point, Vector2 pivot, Quaternion angle)
    {
        ...
        if (Application.isEditor)
        {
            ...
            foreach (List<Vector2> polygon in polygons)
            {
                for (int i = 0; i < polygon.Count; i++)
                {
                    ...
                    {
                        Gizmos
                            .DrawLine(polygon[i] + offset, polygon[0] + offset);
                    }
                    else
                    {
                        Gizmos
                            .DrawLine(polygon[i] + offset,
                            polygon[i + 1] + offset);
                    }
                }
            }
