...
public static class SpriteExploder
{
    ...
    GenerateTriangularPieces(
        GameObject source,
        int extraPoints = 0,
        int subshatterSteps = 0,
        Material mat = null
    )
    {
        ...
        PolygonCollider2D sourcePolyCollider =
        ...
        {
            points
                .Add(new Vector2(Random.Range(rect.width / -2, rect.width / 2),
                    Random.Range(rect.height / -2, rect.height / 2)));
        }
        ...
        foreach (Triangle tri in voronoi.Triangles())
        {
            ...
            {
                pieces
                    .Add(generateTriangularPiece(source,
                    triangle,
                    origVelocity,
                    origScale,
                    origRotation,
        ...
        if (subshatterSteps > 0)
        {
            ...
            {
                morePieces
                    .AddRange(SpriteExploder
                        .GenerateTriangularPieces(piece,
                        extraPoints,
                        subshatterSteps,
                        mat));
    ...
    )
    {
        ...
        MeshFilter meshFilter =
            (MeshFilter) piece.AddComponent(typeof (MeshFilter));
    ...
    )
    {
        ...
        foreach (List<Vector2> region in voronoi.Regions())
        {
            ...
            {
                pieces
                    .Add(generateVoronoiPiece(source,
                    clippedRegion,
                    origVelocity,
                    origScale,
                    origRotation,
    ...
    )
    {
        ...
        //add script for destroy by time (fade it out eventually)
        return piece;
    }
    ...
    private static List<Vector2> getPoints(BoxCollider2D collider)
    {
        ...
        //bottom left
        points
            .Add(new Vector2((center.x - size.x / 2), (center.y - size.y / 2)));
    ...
    private static List<Vector2> getRendererPoints(GameObject source)
    {
        ...
        points
            .Add(new Vector2(bounds.center.x - bounds.extents.x,
    ...
    private static Rect getRect(GameObject source)
    {
        ...
        return new Rect(bounds.extents.x * -1,
            bounds.extents.y * -1,
            bounds.size.x,
            bounds.size.y);
    }
    ...
    calcUV(Vector3[] vertices, SpriteRenderer sRend, Transform sTransform)
    {
        float texHeight =
            (sRend.bounds.extents.y * 2) / sTransform.localScale.y;
        float texWidth = (sRend.bounds.extents.x * 2) / sTransform.localScale.x;
        Vector3 botLeft =
            sTransform
                .InverseTransformPoint(new Vector3(sRend.bounds.center.x -
                    sRend.bounds.extents.x,
                    sRend.bounds.center.y - sRend.bounds.extents.y,
                    0));
    ...
    private static Vector2[]
    calcUV(Vector3[] vertices, MeshRenderer mRend, Transform sTransform)
    {
        float texHeight =
            (mRend.bounds.extents.y * 2) / sTransform.localScale.y;
        float texWidth = (mRend.bounds.extents.x * 2) / sTransform.localScale.x;
        Vector3 botLeft =
            sTransform
                .InverseTransformPoint(new Vector3(mRend.bounds.center.x -
                    mRend.bounds.extents.x,
                    mRend.bounds.center.y - mRend.bounds.extents.y,
                    0));
        Vector2[] uv = new Vector2[vertices.Length];

        Vector2[] sourceUV =
            sTransform.GetComponent<MeshFilter>().sharedMesh.uv;
        Vector2 uvMin;
        Vector2 uvMax;
        getUVRange(out uvMin, out uvMax, sourceUV);
    ...
    private static void getUVRange(
        out Vector2 min,
        out Vector2 max,
        Vector2[] uv
    )
    ...
    private static Vector2[]
    calcPolyColliderPoints(List<Vector2> points, Vector2 offset)
    {
    ...
    private static void centerMeshPivot(GameObject target, Vector3 diff)
    {
        ...
        target.transform.localPosition =
            target.transform.TransformPoint(pivot - diff);
    }
    ...
    private static void setFragmentMaterial(
