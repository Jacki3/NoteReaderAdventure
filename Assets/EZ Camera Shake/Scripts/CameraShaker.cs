...
namespace EZCameraShake
{
    ...
    public class CameraShaker : MonoBehaviour
    {
        ...
        static Dictionary<string, CameraShaker>
        ...
        List<CameraShakeInstance>
            cameraShakeInstances = new List<CameraShakeInstance>();
        ...
        void Update()
        {
            ...
            {
                if (i >= cameraShakeInstances.Count) break;

                CameraShakeInstance c = cameraShakeInstances[i];

                if (
                    c.CurrentState == CameraShakeState.Inactive &&
                    c.DeleteOnInactive
                )
                {
        ...
        public static CameraShaker GetInstance(string name)
        {
            ...
            if (instanceList.TryGetValue(name, out c)) return c;
        ...
        )
        {
            CameraShakeInstance shake =
                new CameraShakeInstance(magnitude,
                    roughness,
                    fadeInTime,
                    fadeOutTime);
        ...
        /// <returns>A CameraShakeInstance that can be used to alter the shake's properties.</returns>
        public CameraShakeInstance
        StartShake(float magnitude, float roughness, float fadeInTime)
        {
            CameraShakeInstance shake =
